...
* Keep focused tests in the component's own `tests/` directory.
* Use `cpro_unity_test()` for new C unit tests. Each test source includes `unity.h`, defines `setUp()` and `tearDown()`, and runs independent cases through `UNITY_BEGIN()`, `RUN_TEST()`, and `UNITY_END()`.
* Application smoke tests use `cpro_smoke_test()` and must start the complete executable with `--help` without requiring credentials, network listeners, databases, or writable installation data.
* Database-facing unit tests may use the test-only `cpro_test_sqlite` driver with a single-connection in-memory pool. Do not make production code depend on SQLite solely for testing.
